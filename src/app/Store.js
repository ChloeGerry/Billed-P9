...
class Api {
  ...
  async post({ url, data, headers }) {
    return jsonOrThrowIfError(
      await fetch(`${this.baseUrl}${url}`, { headers, method: "POST", body: data })
    );
  }
  ...
  async patch({ url, data, headers }) {
    return jsonOrThrowIfError(
      await fetch(`${this.baseUrl}${url}`, { headers, method: "PATCH", body: data })
    );
  }
}
...
class Store {
  ...
  login = (data) =>
    this.api.post({ url: "/auth/login", data, headers: getHeaders({ noAuthorization: true }) });
